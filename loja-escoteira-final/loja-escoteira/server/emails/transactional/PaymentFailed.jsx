...
export function PaymentFailed({ customerName, orderNumber, total, paymentUrl, failureReason }) {
  return (
    <EmailLayout preview={`Pagamento não aprovado - Pedido #${orderNumber}`} title="Pagamento não aprovado" subtitle={`Pedido #${orderNumber}`}>
      <EmailStatusBadge tone="danger">FALHA NO PAGAMENTO</EmailStatusBadge>
      <Text>Olá, {customerName || "cliente"}.</Text>
      <Text>Não conseguimos confirmar o pagamento da sua compra. Revise os dados e tente novamente para manter os itens do pedido.</Text>
